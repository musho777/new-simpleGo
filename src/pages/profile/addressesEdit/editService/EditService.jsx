...
const EditService = ({ onEdit, onCancel, isEdit, showEditAddButton }) => {
  const data = useSelector(selectAddresses);
  const serviceEmailData = data.service;
  const { uuid } = useParams();
  const userType = localStorage.getItem('userType');

  const initialData = serviceEmailData
  ...
  const {
    ...
    defaultValues: {
      addresses: isEdit === 'service' ? initialData : [],
    },
  });
  ...
  const formData = watch('addresses');

  useEffect(() => {
    const isFormChanged = formData.some((address, index) => {
  ...
  }, [formData, initialData, delatedAddress]);
  ...
  const onSubmit = (formData) => {
    ...
    if (updatedAddresses.length > 0) {
      if (showEditAddButton === 'edit') {
        dispatch(
          editAddress({ type: 'service', addressChanges: updatedAddresses, userType, uuid })
        );
      }

      if (showEditAddButton === 'add') {
        dispatch(
          createAddress({ type: 'service', addresses: updatedAddresses, userType, uuid })
        );
      }
    }
  ...
  const handleAddAddress = () => {
    ...
    if (rowCount < 3 - data?.service?.length) {
      append({
        address1: '',
        address2: '',
        city: '',
  ...
  return (
    <Form onSubmit={handleSubmit(onSubmit)} onKeyDown={handleKeyDown}>
        ...
        <Container key={field.id}>
          <Group>
            ...
            {showEditAddButton === 'edit' && (
              <TrashWrapper onClick={() => handleRemoveRow(index)} disabled={field.pending}>
      ...
      {rowCount < 3 - data?.service?.length && isEdit !== 'service' && (
        <AddAddress onClick={handleAddAddress}>+ Add service address</AddAddress>
      )}
      {showEditAddButton && (
        <ButtonContainer>
          ...
          <Button type="submit" secondary disabled={!isChanged}>
            Save changes
          </Button>
        </ButtonContainer>
      )}
