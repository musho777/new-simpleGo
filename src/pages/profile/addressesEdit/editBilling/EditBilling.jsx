...
const EditBilling = ({ onEdit, onCancel, isEdit, showEditAddButton }) => {
  const data = useSelector(selectAddresses);
  const serviceEmailData = data.billing;
  ...
  const {
    ...
    defaultValues: {
      addresses: isEdit === 'billing' ? initialData : [],
    },
  });
  ...
  const onSubmit = (formData) => {
    ...
    if (updatedAddresses.length > 0) {
      if (showEditAddButton === 'edit') {
        dispatch(
          editAddress({ type: 'billing', addressChanges: updatedAddresses, userType, uuid })
        );
      }

      if (showEditAddButton === 'add') {
        dispatch(
          createAddress({ type: 'billing', addresses: updatedAddresses, userType, uuid })
        );
      }
    }
  ...
  const handleAddAddress = () => {
    ...
    if (rowCount < 3 - data?.billing?.length) {
      append({
        address1: '',
        address2: '',
        city: '',
  ...
  return (
    <Form onSubmit={handleSubmit(onSubmit)} onKeyDown={handleKeyDown}>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Input
                    ...
                    error={errors?.addresses?.[index]?.address2?.message}
                  />
                )}
              />
            </Row>
      ...
      {rowCount < 3 - data?.billing?.length && isEdit !== 'billing' && (
        <AddAddress onClick={handleAddAddress}>+ Add billing address</AddAddress>
      )}
      {showEditAddButton && (
        <ButtonContainer>
