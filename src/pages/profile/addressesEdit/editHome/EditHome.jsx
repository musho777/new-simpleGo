...
const EditHome = ({ onEdit, onCancel, isEdit, showEditAddButton }) => {
  const data = useSelector(selectAddresses);
  const homeEmailData = data.home;
  const { uuid } = useParams();
  const userType = localStorage.getItem('userType');

  const initialData = homeEmailData
  ...
  const {
    ...
    defaultValues: {
      addresses: isEdit === 'home' ? initialData : [],
    },
  });
  ...
  const onSubmit = (formData) => {
    ...
    if (updatedAddresses.length > 0) {
      if (showEditAddButton === 'edit') {
        dispatch(
          editAddress({ type: 'home', addressChanges: updatedAddresses, userType, uuid })
        );
      }

      if (showEditAddButton === 'add') {
        dispatch(createAddress({ type: 'home', addresses: updatedAddresses, userType, uuid }));
      }
    }
  ...
  const handleAddAddress = () => {
    ...
    if (rowCount < 3 - data?.home?.length) {
      append({
        address1: '',
        address2: '',
        city: '',
  ...
  const handleRemoveRow = (index) => {
    ...
    if (addressId) {
      ...
      // dispatch(deleteAddress({ addressIds: [addressId], userType, uuid }));
    }
  ...
  useEffect(() => {
    const subscription = watch((value) => {
      const isModified = value.addresses.some((address, index) => {
        const initialAddress = initialData?.find(
  ...
  return (
    <Form onSubmit={handleSubmit(onSubmit)} onKeyDown={handleKeyDown}>
      ...
      {rowCount < 3 - data?.home?.length && isEdit !== 'home' && (
        <AddAddress onClick={handleAddAddress}>+ Add home address</AddAddress>
      )}
      {showEditAddButton && (
        <ButtonContainer>
          ...
          <Button type="submit" secondary disabled={!isSaveEnabled}>
            Save changes
          </Button>
        </ButtonContainer>
      )}
