...
const Billing = () => {
  ...
  const data = newAddresses.update.billing;

  const billingAddressData = data.map((item) => {
    return {
      uuid: item.uuid,
  ...
  useEffect(() => {
    dispatch(setSelectedNewAddresses({ billing: selectedItems }));
  }, [selectedItems]);
