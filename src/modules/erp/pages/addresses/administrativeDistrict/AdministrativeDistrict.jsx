...
import {
  AdministrativeDistrictContainer,
  AdministrativeDistrictItem,
  AdministrativeDistrictName,
  AdministrativeDistrictsList,
  ButtonWrapper,
  IconButton,
  IconsContainer,
  PaginationWrapper,
  ...
  SelectContainer,
  SelectWrapper,
} from './AdministrativeDistrict.styles';
import {
  createAdministrativeDistrictCreateSchema,
  createAdministrativeDistrictFormSchema,
} from './schema';
import { useAdministrativeDistrictSearchParams } from './useAdministrativeDistrictSearchParams';

const AdministrativeDistrict = () => {
  const dispatch = useDispatch();
  const { administrativeDistricts, countries, regions, cities, loading } = useSelector(
    (state) => state.main
  );
  const { searchData, setAdministrativeDistrictSearchData } =
    useAdministrativeDistrictSearchParams();
  const {
    currentPage,
    limit: rowsPerPage,
    selectedCountry,
  ...
  const formResolver = useMemo(
    () =>
      yupResolver(
        isEditMode
          ? createAdministrativeDistrictFormSchema(administrativeDistricts)
          : createAdministrativeDistrictCreateSchema(administrativeDistricts)
      ),
    [administrativeDistricts, isEditMode]
  );
  ...
  const onSubmit = async (data) => {
    ...
    if (selectedCity) {
      dispatch(getAdministrativeDistricts({ cityId: selectedCity.value }));
    }
    handleCloseModal();
  };
  ...
  const handleEdit = (district) => {
    ...
    setValue('country', district.country || null);
    setValue('region', district.region || null);
    setValue('city', district.city || null);
    setValue('id', district.id);
    setModalCountry(district.country || null);
    setModalRegion(district.region || null);
    setIsModalOpen(true);
  };
  ...
  const handleConfirmDelete = async () => {
    if (districtToDelete) {
      const result = await dispatch(deleteAdministrativeDistrict(districtToDelete.id));

      if (deleteAdministrativeDistrict.rejected.match(result)) {
  ...
  const filteredDistricts = selectedCity ? administrativeDistricts : [];
  ...
  const handlePageChange = (page) => {
    setAdministrativeDistrictSearchData({ currentPage: page });
  };

  const handleRowCountChange = (newRowCount) => {
    setAdministrativeDistrictSearchData({ limit: newRowCount, currentPage: 1 });
  };
  ...
  const handleCityChange = (selectedOption) => {
    ...
    setAdministrativeDistrictSearchData({ selectedCity: selectedOption, currentPage: 1 });
  };
    ...
    <AdministrativeDistrictContainer>
      ...
      <AdministrativeDistrictsList>
        {currentDistricts.map((district) => (
          <AdministrativeDistrictItem key={district.id}>
            <div>
              <AdministrativeDistrictName>{district.name}</AdministrativeDistrictName>
            </div>
            <IconsContainer>
              <IconButton
                onClick={() => handleEdit(district)}
                disabled={
                  loading.updateAdministrativeDistrict || loading.deleteAdministrativeDistrict
                }
              >
      ...
      {filteredDistricts.length > 10 && (
        <PaginationWrapper>
          <Pagination
            ...
            dataCount={filteredDistricts.length}
            count={rowsPerPage}
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <Controller
            ...
            render={({ field }) => (
              <Input
                {...field}
                label={isEditMode ? 'Խմբագրել թաղամաս' : 'Ստեղծել վարչական շրջան'}
                placeholder="Վարչական շրջանի անունը"
                required
          ...
          <Row>
            <Button
              ...
              disabled={
                loading.addAdministrativeDistrict || loading.updateAdministrativeDistrict
              }
            >
              Չեղարկել
            </Button>
      ...
      >
        ...
        >
          Համոզված եք,որ ցանկանում եք ջնջել այս թաղամասը
        </p>
        <Row>
      ...
      >
        ...
        >
          Դուք չեք կարող ջնջել այս թաղամասը, քանի որ այս թաղամասի տակ կան շենքեր/տներ
        </p>
        <div
          style={{
            display: 'flex',
