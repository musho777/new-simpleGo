...
import { getExportPaymentReport } from 'modules/billing/features/main/mainActions';
import { setExportPaymentSearchData } from 'modules/billing/features/main/mainSlice';
import { billingTypes } from 'modules/billing/pages/subscribers/billingConstants';
...
const FilterActions = ({ searchData, resetFilters, handleResetAllFilters }) => {
  ...
  const handleFilterChange = (key, value) => {
    ...
    dispatch(setExportPaymentSearchData(updatedSearchData));
  };
  ...
  useEffect(() => {
    if (from && to && currentStatus && oldStatus && currentAbsentStatus && secret) {
      dispatch(
        getExportPaymentReport({
          ...searchData,
          secret,
  ...
  }, [searchData, from, to, currentStatus, oldStatus, currentAbsentStatus, dispatch, secret]);
  ...
  return (
    <FilterWrapper>
      <Title>Ներբեռնել վճարումների հաշվետվությունը</Title>
      <FilterActionsWrapper>
        <DatePicker
