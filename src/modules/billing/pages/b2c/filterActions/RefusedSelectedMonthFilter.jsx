...
import { getRefusedSelectedMonthReport } from 'modules/billing/features/main/mainActions';
...
const RefusedSelectedMonthFilter = () => {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const result = await dispatch(getRefusedSelectedMonthReport(params));
      ...
      if (result.payload && result.payload instanceof Blob) {
        ...
        link.setAttribute(
          'download',
          `refused-selected-month-${selectedMonth}-${new Date().toISOString().split('T')[0]}.xlsx`
        );
  ...
  return (
    <FilterWrapper>
      <Title>Հրաժարված բաժանորդների նախորդ կարգավիճակ</Title>
      <FilterActionsWrapper>
        <DatePickerWrapper>
          <CustomDatePicker
            value={selectedMonth}
...
export default RefusedSelectedMonthFilter;
