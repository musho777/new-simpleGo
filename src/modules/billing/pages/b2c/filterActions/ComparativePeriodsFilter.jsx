...
import { getComparativePeriodsReport } from 'modules/billing/features/main/mainActions';
...
const ComparativePeriodsFilter = () => {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const result = await dispatch(getComparativePeriodsReport(params));
      ...
      if (result.payload && result.payload instanceof Blob) {
        ...
        link.setAttribute(
          'download',
          `comparative-periods-${dateFrom}-to-${dateTo}-${new Date().toISOString().split('T')[0]}.xlsx`
        );
