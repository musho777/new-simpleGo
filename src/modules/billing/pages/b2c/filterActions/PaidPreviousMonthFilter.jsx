...
import { getPaidPreviousMonthReport } from 'modules/billing/features/main/mainActions';
...
const PaidPreviousMonthFilter = () => {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const result = await dispatch(getPaidPreviousMonthReport(params));
      ...
      if (result.payload && result.payload instanceof Blob) {
        ...
        link.setAttribute(
          'download',
          `paid-previous-month-${selectedMonth}-${new Date().toISOString().split('T')[0]}.xlsx`
        );
  ...
  return (
    <FilterWrapper>
      <Title>Նախորդ ամիս վճարում կատարած բաժանորդներ</Title>
      <FilterActionsWrapper>
        <DatePickerWrapper>
          <CustomDatePicker
            views={['month', 'year']}
...
export default PaidPreviousMonthFilter;
