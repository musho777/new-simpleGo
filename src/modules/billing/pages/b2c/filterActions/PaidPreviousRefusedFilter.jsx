...
import { getPaidPreviousRefusedReport } from 'modules/billing/features/main/mainActions';
...
const PaidPreviousRefusedFilter = () => {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const result = await dispatch(getPaidPreviousRefusedReport(params));
      ...
      if (result.payload && result.payload instanceof Blob) {
        ...
        link.setAttribute(
          'download',
          `paid-previous-refused-${selectedMonth}-${new Date().toISOString().split('T')[0]}.xlsx`
        );
  ...
  return (
    <FilterWrapper>
      <Title>Նախորդ ամիս վճարած այժմ հրաժարված բաժանորդներ</Title>
      <FilterActionsWrapper>
        <DatePickerWrapper>
          <CustomDatePicker
            value={selectedMonth}
...
export default PaidPreviousRefusedFilter;
