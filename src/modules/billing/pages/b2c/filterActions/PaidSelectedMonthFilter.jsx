...
import { getPaidSelectedMonthReport } from 'modules/billing/features/main/mainActions';
...
const PaidSelectedMonthFilter = () => {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const result = await dispatch(getPaidSelectedMonthReport(params));
      ...
      if (result.payload && result.payload instanceof Blob) {
        ...
        link.setAttribute(
          'download',
          `paid-selected-month-${selectedMonth}-${new Date().toISOString().split('T')[0]}.xlsx`
        );
  ...
  return (
    <FilterWrapper>
      <Title>Այս ամիս վճարած բաժանորդների նախորդ կարգավիճակ</Title>
      <FilterActionsWrapper>
        <DatePickerWrapper>
          <CustomDatePicker
            value={selectedMonth}
...
export default PaidSelectedMonthFilter;
